...
export default defineConfig(({ command, mode }) => {
  // 解决 monorepo 下 wot-design-uni 依赖 hoisting 导致 build 报错的问题
  // 自动创建软链接，避免手动执行脚本
  ...
  if (fs.existsSync(target) && !fs.existsSync(linkPath)) {
    try {
      ...
      console.log('[vite] Symlinked wot-design-uni for monorepo support')
    }
    catch (e) {
      console.error('[vite] Failed to symlink wot-design-uni', e)
    }
  }
  ...
  // bun dev:h5 时得到 => serve development
  // bun build:h5 时得到 => build production
  // bun dev:mp-weixin 时得到 => build development (注意区别，command为build)
  // bun build:mp-weixin 时得到 => build production
  // bun dev:app 时得到 => build development (注意区别，command为build)
  // bun build:app 时得到 => build production
  ...
  return defineConfig({
    ...
    plugins: [
      ...
      UniPages({
        ...
        subPackages: [
          'src/subpackage/pages-fg', // 这个是相对必要的路由（登录页、注册页、404页等）
          'src/subpackage/pages-me', // 我的相关页面（个人中心、设置等）
          // 'src/subpackage/pages-main', //  主流程相关页面
        ],
