...
export namespace AddressModel {
  ...
  export const listParams = z.object({
    pageNum: z.string().optional().transform(val => val ? Number.parseInt(val) : 1).describe('当前页'),
    pageSize: z.string().optional().transform(val => val ? Number.parseInt(val) : 10).describe('每页大小'),
  }).describe('地址列表查询参数')

  export const listParamsSchema = z.object({
    pageNum: z.string().transform(val => Number.parseInt(val)).default(1).describe('当前页'),
    pageSize: z.string().transform(val => Number.parseInt(val)).default(10).describe('每页大小'),
  })
