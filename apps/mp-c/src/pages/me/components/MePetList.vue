...
<template>
  <view class="mt-8 animate-fade-in-up px-6 delay-200">
    <view class="mb-4 flex items-end justify-between">
      <view class="relative">
        <text class="relative z-10 text-xl text-[#333] font-black">毛孩子</text>
        <view class="-z-0 absolute bottom-1 left-0 h-3 w-full rounded-full bg-[#ff7a00] opacity-20" />
      </view>
    ...
    <scroll-view scroll-x class="whitespace-nowrap px-6 pb-6 -mx-6" :show-scrollbar="false">
      <view class="flex gap-4">
        ...
        <!-- 添加卡片 -->
        <view
          class="box-border h-320rpx w-240rpx flex flex-shrink-0 flex-col items-center justify-center gap-3 border-2 border-[#E0E0E0] rounded-[32rpx] border-dashed bg-[#FAFAFA] p-4 transition-colors active:bg-[#F5F5F5]"
          @click="toastAdd"
        >
